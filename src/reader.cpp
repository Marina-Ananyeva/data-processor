...
ElementType Reader::readElemType(const std::string &elemName) const
{
    if (elemName == "int")
...
OperationType Reader::readOperationType(const std::string &operationName) const
{
...
std::unique_ptr<Element> Reader::parseNumber(std::istream& input) const 
{
...
std::unique_ptr<Element> Reader::parseStringToElement(const std::string &data) const
{
    try {
...
std::vector<std::pair<OperationType, Data>> Reader::readOperations(const json::Array &operations) 
{
